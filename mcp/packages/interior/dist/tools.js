 ...
 */
import { generateRoomPlanner, ROOM_TYPES, WALL_STYLES, FLOOR_STYLES, } from "./room-planner.js";
import { generateFurniturePlacement, FURNITURE_CATEGORIES, FURNITURE_SIZES, } from "./furniture-placement.js";
import { generateMaterialSwitcher, MATERIAL_SURFACES, PAINT_FINISHES, } from "./material-switcher.js";
import { generateLightingDesign, LIGHT_TYPES, COLOR_TEMPERATURES, } from "./lighting-design.js";
import { generateRoomTour, TOUR_STYLES, TOUR_SPEEDS, } from "./room-tour.js";
import { listFurnitureModels, formatFurnitureModelList, FURNITURE_MODEL_CATEGORIES, } from "./furniture-models.js";
...
export const TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        description: "Validates a Kotlin SceneView snippet for common interior-design app mistakes. Checks threading violations (Filament JNI on background thread), null-safety for model loading, LightNode trailing-lambda bug, deprecated 2.x APIs, unsupported format warnings (FBX, OBJ, 3DS), texture size warnings, and multiple model instance performance. Always call this before presenting generated interior SceneView code.",
        inputSchema: {
            type: "object",
            properties: {
                code: {
    ...
    switch (toolName) {
        ...
        case "get_room_planner": {
            ...
            const code = generateRoomPlanner({
                ...
                lengthMeters: args.lengthMeters,
                heightMeters: args.heightMeters,
                wallStyle: args.wallStyle,
        ...
        case "get_furniture_placement": {
            ...
            const code = generateFurniturePlacement({
                ...
                rotatable: args.rotatable ?? true,
                scalable: args.scalable ?? true,
                ar: args.ar ?? true,
            });
            const mode = (args.ar ?? true) ? "AR" : "3D";
            return {
                content: withDisclaimer([
                    {
                        type: "text",
        ...
        case "get_material_switcher": {
            ...
            const code = generateMaterialSwitcher({
                ...
                finish: args.finish,
                colors: args.colors,
                texturePaths: args.texturePaths,
                beforeAfter: args.beforeAfter ?? false,
                ar: args.ar ?? false,
            });
            const mode = args.ar ? "AR" : "3D";
            return {
        ...
        case "get_lighting_design": {
            ...
            const code = generateLightingDesign({
                lights,
                colorTemperature: args.colorTemperature,
                dimmable: args.dimmable ?? true,
                roomModel: args.roomModel,
                showShadows: args.showShadows ?? true,
                ar: args.ar ?? false,
            });
            const mode = args.ar ? "AR" : "3D";
            return {
        ...
        case "get_room_tour": {
            const args = rawArgs ?? {};
            const tourStyle = args.tourStyle;
            ...
            const code = generateRoomTour({
                ...
                loop: args.loop ?? true,
                pauseOnInteraction: args.pauseOnInteraction ?? true,
                ar: args.ar ?? false,
            });
            const mode = args.ar ? "AR" : "3D";
            return {
        ...
        default:
            return {
                content: [
                    {
                        type: "text",
                        text: `Unknown tool: "${toolName}". Available: get_room_planner, get_furniture_placement, get_material_switcher, get_lighting_design, get_room_tour, list_furniture_models, validate_interior_code`,
                    },
                ],
                isError: true,
            };
