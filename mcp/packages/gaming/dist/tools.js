...
import { generatePhysicsGame, PHYSICS_PRESETS, GRAVITY_MODES, } from "./physics-game.js";
import { generateParticleEffects, PARTICLE_EFFECTS, BLEND_MODES, } from "./particle-effects.js";
import { generateInventory3D, ITEM_CATEGORIES, INVENTORY_LAYOUTS, } from "./inventory-3d.js";
import { listGameModels, formatModelList, GAME_MODEL_CATEGORIES, } from "./game-models.js";
...
export const TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        description: "Lists free, openly-licensed 3D game models suitable for SceneView apps. Sources include Khronos glTF Sample Models (CC BY/CC0), Kenney.nl (CC0 Public Domain), Quaternius (CC0), and Sketchfab (CC BY). Categories: character, weapon, vehicle, environment, prop, creature, building, item, effect, ui. Each entry includes source URL, format, license, and SceneView compatibility notes.",
        inputSchema: {
            type: "object",
            properties: {
                category: {
    ...
    },
    {
        ...
        description: "Validates a Kotlin SceneView snippet for common gaming-app mistakes. Checks threading violations (Filament JNI on background thread), null-safety for model loading, LightNode trailing-lambda bug, deprecated 2.x APIs, physics timestep issues, particle performance, animation safety, and Random seeding. Always call this before presenting generated gaming SceneView code.",
        inputSchema: {
            type: "object",
            properties: {
                code: {
    ...
    switch (toolName) {
        ...
        case "get_character_viewer": {
            ...
            const code = generateCharacterViewer({
                style,
                animations: args.animations ?? ["idle", "walk", "run"],
                autoRotate: args.autoRotate ?? false,
                showControls: args.showControls ?? true,
                ar: args.ar ?? false,
            });
        ...
        case "get_level_editor": {
            ...
            const code = generateLevelEditor({
                theme,
                geometries: args.geometries ?? ["cube", "sphere", "cylinder", "plane"],
        ...
        case "get_physics_game": {
            ...
            const code = generatePhysicsGame({
                preset,
                gravity: args.gravity ?? "earth",
                bounciness: args.bounciness ?? 0.7,
                showTrajectory: args.showTrajectory ?? false,
                ar: args.ar ?? false,
            });
            const mode = args.ar ? "AR" : "3D";
            return {
        ...
        case "get_inventory_3d": {
            ...
            const code = generateInventory3D({
                layout,
                categories: args.categories ?? ["weapon", "armor", "potion", "gem"],
        ...
        default:
            return {
                content: [
                    {
                        type: "text",
                        text: `Unknown tool: "${toolName}". Available: get_character_viewer, get_level_editor, get_physics_game, get_particle_effects, get_inventory_3d, list_game_models, validate_game_code`,
                    },
                ],
                isError: true,
            };
